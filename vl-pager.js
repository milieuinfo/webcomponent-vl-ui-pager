import{VlElement,define}from"/node_modules/vl-ui-core/vl-core.js";import{html,render}from"/node_modules/lite-html/lite-html.js";export class VlPager extends VlElement(HTMLElement){static get _observedAttributes(){return["total-items","current-page","items-per-page"]}static get _observedChildClassAttributes(){return["align-center","align-right"]}get _classPrefix(){return"vl-pager--"}constructor(){super(`<style>
           ...
           <div class="vl-pager">
            <ul class="vl-pager__list">
              ...
              <li id="pageForwardListItem" class="vl-pager__element">
           ...
           </div>`)}connectedCallback(){if(!this._dressed){this.shadowRoot.querySelector("#pageBackLink").addEventListener("click",()=>{if(!(this.currentPage-1<=0)){this.setAttribute("current-page",this.currentPage-1)}});this.shadowRoot.querySelector("#pageForwardLink").addEventListener("click",()=>{if(!(this.currentPage+1>this.totalPages)){this.setAttribute("current-page",this.currentPage+1)}});this._dressed=true}}attributeChangedCallback(name,oldValue,newValue){super.attributeChangedCallback(name,oldValue,newValue);this.currentPage===this.totalPages?this._hide(this._pageForwardListItem):this._show(this._pageForwardListItem);this.currentPage===1?this._hide(this._pageBackListItem):this._show(this._pageBackListItem);this._updateItemsInfo();this._updatePagination()}_items_per_pageChangedCallback(oldValue,newValue){}_total_itemsChangedCallback(oldValue,newValue){!parseInt(newValue)?this._hide(this):this._show(this)}_hide(element){element.style.display="none"}_show(element){element.style.display=""}_items_per_pageChangeCallback(oldValue,newValue){}_current_pageChangedCallback(oldValue,newValue){this.dispatchEvent(new CustomEvent("pagechanged",{detail:{oldPage:oldValue,newPage:newValue},bubbles:true}))}_updateItemsInfo(){this.shadowRoot.querySelector("#itemsOfCurrentPageInfo").innerHTML=`${this._firstItemNrOfPage}-${this._lastItemNrOfPage}`;this.shadowRoot.querySelector("#totalItems").innerHTML=this.totalItems;this.shadowRoot.querySelectorAll('[name="itemsPerPage"]').forEach(span=>{span.innerHTML=`${this.itemsPerPage} rijen`})}_updatePagination(){render(this._renderPageLinks(),this.shadowRoot.querySelector("pages-links"))}_renderPageLinks(){const pages=this._calculatePagination(this.currentPage,this.totalPages);return html`${pages.length>1?pages.map(pageNr=>this._renderPageLink(pageNr)):""}`}_renderPageLink(number){if(number===this.currentPage){return html`<li name="pageLink" data-vl-pager-page=${number} class="vl-pager__element"> 
                  <label>${number}</label>
                ...
                </li>`}}get totalPages(){return Math.ceil(this.totalItems/this.itemsPerPage)}get totalItems(){return parseInt(this.getAttribute("total-items"))}get currentPage(){return parseInt(this.getAttribute("current-page"))}get itemsPerPage(){return parseInt(this.getAttribute("items-per-page"))}get _firstItemNrOfPage(){if(!this.totalItems){return 0}else{return(this.currentPage-1)*this.itemsPerPage+1}}get _lastItemNrOfPage(){const lastItemNr=this._firstItemNrOfPage+this.itemsPerPage-1;return lastItemNr>this.totalItems?this.totalItems:lastItemNr}get _pageBackListItem(){return this.shadowRoot.querySelector("#pageBackListItem")}get _pageForwardListItem(){return this.shadowRoot.querySelector("#pageForwardListItem")}_calculatePagination(c,m){var current=c,last=m,delta=1,left=current-delta,right=current+delta+1,range=[],rangeWithDots=[],l;for(let i=1;i<=last;i++){if(i==1||i==last||i>=left&&i<right){range.push(i)}}for(let i of range){if(l){if(i-l===2){rangeWithDots.push(l+1)}else if(i-l!==1){rangeWithDots.push("skipped")}}rangeWithDots.push(i);l=i}return rangeWithDots}};define("vl-pager",VlPager);